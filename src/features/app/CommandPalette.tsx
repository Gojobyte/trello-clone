...
export function CommandPalette({
	...
	const all = useMemo<CmdItem[]>(() => {
		...
		return [
			{
				kind: "page",
				label: "Aller aux tableaux",
				icon: "folder",
				run: nav("/boards"),
			},
			{
				kind: "page",
				label: "Aller à l'inbox",
				icon: "inbox",
				run: nav("/my-work", { tab: "inbox" }),
			},
			{
				kind: "page",
				label: "Mon jour",
				icon: "spark",
				run: nav("/my-work", { tab: "today" }),
			},
			{
				kind: "page",
				label: "Mes tâches",
				icon: "check",
				run: nav("/my-work", { tab: "tasks" }),
			},
			{
				kind: "page",
				label: "Objectifs (OKR)",
		...
		<div className="modal-backdrop" onClick={onClose}>
			<div className="cmdk" onClick={(e) => e.stopPropagation()}>
				...
				<div className="cmdk-footer">
					...
					<span className="cmdk-footer-brand">640px · Lume</span>
				</div>
			</div>
		</div>
	);
